...
ROLE_MATRIX = [
    ...
    ['School Review', 'View', 'Edit (Leader/Admin only)', 'No', 'View Self-Evaluation + ask questions', 'No'],
...
MODULES = [
    ...
    },
    {
        ...
        'purpose': 'The School Leader/Admin\'s entry point into the Yarra School Self-Evaluation Record -- nothing else lives on this page by design.',
        'roles': 'School Leader, Admin only -- not available to Teachers, not even to view.',
        'workflow': [
            'A review record is created automatically the first time School Leader/Admin opens the page -- there is no manual "create a cycle" step.',
            'Four progress cards (Part A-D, see Module 21) show Not Started / In Progress / Complete, each with a Start/Continue/Review button into that Part.',
            'Any open Yarra Evaluator questions about the Self-Evaluation Record appear here for School Leader/Admin to answer.',
        ],
        'stories': [
            'As a School Leader, I want to see at a glance which Parts of our Self-Evaluation Record still need work.',
            'As a School Admin, I want to see and answer a Yarra Evaluator\'s question right here, without a separate email thread.',
        ],
        'acceptance': [
            'A Teacher hitting this page (even by direct URL) gets a 403, not a read-only view.',
        ],
    },
    {
    ...
    },
    {
        ...
        'purpose': 'Reproduce Ms Chelli\'s official Self-Evaluation Record in-app -- ~170 questions across 4 Parts (A: School Profile & Data, B: Institutional Capability Review across 5 capability areas, C: Supporting Evidence, D: School Priorities) -- so a school\'s answers are visible to their Yarra Evaluator without a separate document, and so schools aren\'t forced to complete all ~170 questions in one sitting.',
        'roles': 'School Leader, Admin (fill out). Yarra Evaluator (view, read-only). Not available to Teachers.',
        'workflow': [
            'From School Review, open one Part at a time (A, B, C, or D) -- each saves independently.',
            'Answer questions grouped by Section -> Sub-area, matching the original form exactly: short answer, paragraph, single/multi-choice, 1-4 ratings, and evidence file uploads.',
            'Saving a Part returns to School Review, which shows Not Started / In Progress / Complete for every Part -- pick up any Part later exactly where it was left off.',
            'The Yarra Evaluator sees per-Part completion plus the full record read-only (with any uploaded evidence files) from their school view.',
        ],
        'stories': [
            'As a School Admin, I want to fill out one Part today and the rest next week without losing what I already saved.',
            'As a Yarra Evaluator, I want to see how much of a school\'s Self-Evaluation Record is actually complete before I review it.',
        ],
        'acceptance': [
            'Teachers cannot open School Review or the Self-Evaluation Record even by direct URL -- both return 403.',
            'Saving one Part never touches another Part\'s already-saved answers.',
